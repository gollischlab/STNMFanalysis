...
for i=1:option.iter
    ...
    Ae=[A;sqrt(option.beta)*ones(1,k)]; % Use an extended matrix of weights to integrate the sparsity term
    X0=[X;zeros(1,c)];                  % Also extended data patrix
    Y=kfcnnls4STNMF(Ae,X0);                   % Call a function for solving the non-negative least-squares problem for optimizing the modules for fixed weights
